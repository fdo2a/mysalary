...
function table(rowsToRender, { compact = false, grouped = !compact } = {}) {
  const columnCount = compact ? 10 : 11;
  ...
  const body = rowsToRender
    .map((row) => {
      ...
      const groupBreak =
        grouped && hasRenderedRows && previousGroupStart !== groupStart
          ? `<tr class="table-ad-row"><td colspan="${columnCount}"><div class="table-ad-slot">AdSense 광고 영역</div></td></tr>
          <tr class="repeat-header">${headerCells}</tr>`
          : "";
      ...
      const mobileAnnual = compact
        ? ""
        : `<div><span>연실수령</span><strong>${formatKRW(row.netAnnual)}</strong></div>`;
      const mobileCard = `<tr class="mobile-row">
        <td colspan="${columnCount}">
          <article class="mobile-pay-card">
            <header>
              <a href="${salaryHref(row.manwon)}">${salaryTableLabel(row.manwon)}</a>
              <strong>${formatKRW(row.netMonthly)}</strong>
            </header>
            <div><span>월세전</span><strong>${formatKRW(row.grossMonthly)}</strong></div>
            <div><span>국민</span><strong class="negative">${minusKRW(deductions.nationalPension)}</strong></div>
            <div><span>건강</span><strong class="negative">${minusKRW(deductions.healthInsurance)}</strong></div>
            <div><span>요양</span><strong class="negative">${minusKRW(deductions.longTermCare)}</strong></div>
            <div><span>고용</span><strong class="negative">${minusKRW(deductions.employmentInsurance)}</strong></div>
            <div><span>소득세</span><strong class="negative">${minusKRW(deductions.incomeTax)}</strong></div>
            <div><span>지방세</span><strong class="negative">${minusKRW(deductions.localIncomeTax)}</strong></div>
            <div><span>공제합</span><strong class="negative">${minusKRW(row.totalDeductionMonthly)}</strong></div>
            <div><span>월실수령</span><strong>${formatKRW(row.netMonthly)}</strong></div>
            ${mobileAnnual}
          </article>
        </td>
      </tr>`;

      return `${groupBreak}<tr class="salary-row" title="${escapeHtml(summary)}">
